...
%
%%
function Fig4A_2d(cellTable,cellCond)
...
for jj = 1:2:3
    ...
    writematrix([transpose(plotDist) respAveTight(:,jj) respStdErrTight(:,jj)], sprintf("figure_data_2d/fig_4a/compact_%s.csv", kind));
    ...
    writematrix([transpose(plotDist) respAveLoose(:,jj) respStdErrLoose(:,jj)], sprintf("figure_data_2d/fig_4a/spreadout_%s.csv", kind));

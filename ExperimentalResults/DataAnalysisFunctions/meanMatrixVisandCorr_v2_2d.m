function [All, outVars] = meanMatrixVisandCorr_v2_2d(All,opts,outVars);
%% Make all dataPlots into matrixes of mean responses
%%Determine Vis Responsive and Process Correlation
visAlpha =  opts.visAlpha;% 0.05;

if isfield(opts,'skipVis')
    skipVis =opts.skipVis;
else
    skipVis=0;
end
...
for ind=1:numExps
    ...
    for i=1:numel(us)
        ...
        for k= 1 : numel(vs)
            v = vs(k);
            if v~=0
                respMat(i,v,:) = nanmean(All(ind).out.exp.rdData(:,...
                    trialsToUse & All(ind).out.exp.stimID ==s &...
                    All(ind).out.exp.visID ==v), 2) ;
                baseMat(i,v,:) = nanmean(All(ind).out.exp.bdata(:,...
                    trialsToUse & All(ind).out.exp.stimID ==s &...
                    All(ind).out.exp.visID ==v), 2) ;
                stdMat(i,v,:) = nanstd(All(ind).out.exp.rdData(:,...
                    trialsToUse & All(ind).out.exp.stimID ==s &...
                    All(ind).out.exp.visID ==v),[], 2) ;
    ...
    for i=1:size(stimCoM,1)
        for k=1:numCells
            ...
            % use 2D distance
            StimDistance(i,k) = sqrt(sum((stimLoc(i,1:2)-allLoc(k,1:2)).^2));
        end
    end
    ...
    if skipVis
        pVisR = nan([1 All(ind).out.anal.numCells]);
        All(ind).out.anal.pVisR = pVisR; % nan([1 All(ind).out.anal.numCells])
        All(ind).out.anal.visPercent=0;
        visPercent(ind) = 0;
        ...
        for i= 1:numStims
            ...
            if i~=1 && holo~=0
                ...
                for d = 1:numel(distBins)-1
                    ...
                    popRespDist(i,v,d) = nanmean(squeeze(respMat(i,v,cellsToUse) - baseMat(i,v,cellsToUse)));
                    popRespDistNumCells(i,v,d) = sum(cellsToUse);
                    noHoloPopResponse = nanmean(squeeze(respMat(1,v,cellsToUse) - baseMat(1,v,cellsToUse)));
                    ...
                    tempResp = nanmean(squeeze(respMat(i,v,cellsToUse) - baseMat(i,v,cellsToUse)));
                    noHoloPopResponse = nanmean(squeeze(respMat(1,v,cellsToUse) - baseMat(1,v,cellsToUse)));

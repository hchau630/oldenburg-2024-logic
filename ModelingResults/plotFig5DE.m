...
%%
xstart = 100; xend = 350;
figure('Position',[584 553 1033 362]); clf; subplot(1,2,1); hold on;
...
hleg(1) = plot(actDist,aveExc,'.','markersize',16,'color',[0 0.447 0.741]);
plot([xstart:0.1:xend],exc_fit.predict([xstart:0.1:xend]'),'-','color',[0 0.447 0.741],'linewidth',1.5)
hleg(2) = plot(actDist,aveInh,'.','markersize',16,'color',[0.85 0.325 0.098]);
plot([xstart:0.1:xend],inh_fit.predict([xstart:0.1:xend]'),'-','color',[0.85 0.325 0.098],'linewidth',1.5)
set(gca,'fontsize',16)
xlabel('Spread of Ensemble (um)')
ylabel('Input')
xlim([100 350])
...
fprintf('Fig. 5D: Percent difference in slope=%.2f \n',perDiff)